...
if [ -d "$INSTALLER_DIR" ] && [ -d "$UBUNTU_SERVER_DIR" ]; then
	echo "$INSTALLER_DIR and $UBUNTU_SERVER_DIR are exists."
else
	echo "$INSTALLER_DIR and $UBUNTU_SERVER_DIR are not exists."

	if [[ -f "$MAGMA_STATUS_FILE" ]]; then
	        echo "$MAGMA_STATUS_FILE exists"
	else
	        echo "$MAGMA_STATUS_FILE not exist"
	        echo "magma_status=Not" > $MAGMA_STATUS_FILE
	fi
